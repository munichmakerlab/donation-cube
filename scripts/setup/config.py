...
def get_wifi_enabled_choice():
    """Get user choice whether to enable WiFi functionality"""
    colored_print("\n📶 Network Configuration", Colors.BLUE, bold=True)
    colored_print("   WiFi enables MQTT monitoring and remote control", Colors.BLUE)
    colored_print("   Disabling WiFi creates a standalone LED controller", Colors.BLUE)
    ...
    if not is_interactive_terminal():
        ...
        colored_print("   Defaulting to WiFi enabled - edit Config.h manually if needed", Colors.BLUE)
        return True
    
    print()
    
    # Get WiFi enable/disable choice
    while True:
        try:
            choice = input(f"{Colors.BOLD}Enable WiFi and MQTT? [Y/n]: {Colors.END}").strip().lower()
            if choice in ['y', '', 'yes']:
                return True
            elif choice in ['n', 'no']:
                colored_print("📱 WiFi disabled - creating standalone LED controller", Colors.YELLOW)
                colored_print("   No network features will be available", Colors.BLUE)
                return False
            else:
                colored_print("❌ Please enter y or n", Colors.RED)
        except EOFError:
            colored_print("\n⚠️  EOF detected - defaulting to WiFi enabled", Colors.YELLOW)
            return True
...
def create_credentials_file(wifi_enabled, wifi_ssid, wifi_password, mqtt_server, mqtt_port, mqtt_user, mqtt_password):
...
#define ENABLE_WIFI {1 if wifi_enabled else 0}
#define ENABLE_MQTT {1 if wifi_enabled else 0}
...
def setup_credentials():
    ...
    # Get WiFi enable/disable choice first
    wifi_enabled = get_wifi_enabled_choice()
    ...
    if wifi_enabled:
        ...
        # Use placeholder values for disabled WiFi
        wifi_ssid, wifi_password = "WIFI_DISABLED", "WIFI_DISABLED"
        mqtt_server, mqtt_port, mqtt_user, mqtt_password = "MQTT_DISABLED", 1883, "", ""
        colored_print("✅ WiFi and MQTT disabled - standalone mode activated", Colors.GREEN)
    
    # Create credentials file
    if not create_credentials_file(wifi_enabled, wifi_ssid, wifi_password, mqtt_server, mqtt_port, mqtt_user, mqtt_password):
        return False
